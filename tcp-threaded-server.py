...
clients_lock = Lock()  # ป้องกันปัญหาเวลามีหลาย thread แก้ไข clients พร้อมกัน
...
def handle_client(client_socket, addr):
    ...
    while username is None:
            print("[DEBUG] Calling authenticate function")  # เพิ่มข้อความ debug
            username = authenticate(client_socket, auth_handler)
    if username is None:
            print("[ERROR] Authentication failed, retrying...")  # เพิ่มกรณีล้มเหลว
...
# === โหลดข้อมูลผู้ใช้ทั้งหมดจาก database === เปลี่ยนจาก ตอนแรกใช้txt
def load_users():
    global users
